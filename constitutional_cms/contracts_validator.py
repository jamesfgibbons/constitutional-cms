...
This module is the packaged home of the validator. The repository script
``scripts/validate_contracts.py`` re-exports it, and the CLI exposes it as
``constitutional-cms validate [path]``.
...
def validate_web_conformance(contracts_dir: str) -> list[str]:
    """Run the repository's schema and catalog-reference validator when present.

    The web-conformance validator checks this repository's own fixtures, examples,
    and golden receipts, so it only runs from a checkout. Installed environments
    (pip, uvx) validate local contract directories and honestly skip this step.
    """
    script = Path(contracts_dir).resolve().parent / 'scripts' / 'validate_web_conformance.py'
    if not script.exists():
        print("  - skipped: repository checkout not detected (fixtures and goldens live in the repo)")
        return []
...
def validate_packaged_release() -> list[str]:
    """Validate bundled catalog + schemas without a repository checkout."""
    from jsonschema import Draft202012Validator, FormatChecker

    from .evaluator import load_default_catalog, load_schema

    errors: list[str] = []
    catalog = load_default_catalog()
    schema = load_schema("check_catalog_v1.schema.json")
    validator = Draft202012Validator(schema, format_checker=FormatChecker())
    for error in sorted(validator.iter_errors(catalog), key=lambda item: list(item.path)):
        path = ".".join(str(part) for part in error.path) or "$"
        errors.append(f"bundled catalog {path}: {error.message}")
    checks = catalog.get("checks") or []
    if len(checks) != 19:
        errors.append(f"bundled catalog: expected 19 checks, found {len(checks)}")
    if catalog.get("framework_release") != "v0.5.0":
        errors.append(
            f"bundled catalog: framework_release {catalog.get('framework_release')!r} is not v0.5.0"
        )
    if catalog.get("catalog_version") != "1.0.2":
        errors.append(
            f"bundled catalog: catalog_version {catalog.get('catalog_version')!r} is not 1.0.2"
        )
    schema_root = resources.files("constitutional_cms").joinpath("data", "schemas")
    for item in schema_root.iterdir():
        ...
        try:
            ...
            errors.append(f"bundled schema {item.name}: {exc}")
    return errors


def run_packaged() -> int:
    """Print bundled-release validation and return a process exit code."""
    errors = validate_packaged_release()
    if errors:
        for error in errors:
            print(f"  ✗ {error}")
        print(f"  FAILED: {len(errors)} bundled-release error(s)")
        return 1
    from .evaluator import load_default_catalog

    catalog = load_default_catalog()
    print(
        f"  ✓ CheckCatalogV1 {catalog['catalog_version']} "
        f"(framework {catalog['framework_release']}) · {len(catalog['checks'])} checks"
    )
    print("  ✓ Bundled JSON Schemas parse")
    return 0


def run(contracts_dir: str, check: str = 'all') -> int:
...
def main(argv=None):
    ...
    parser.add_argument('--check', choices=CHECK_CHOICES, default='all', help='Which contract to validate')
    args = parser.parse_args(argv)
    return run(args.contracts_dir, args.check)
