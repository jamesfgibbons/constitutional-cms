...
This module is the packaged home of the reference evaluator. The repository script
``scripts/conformance_evaluator.py`` re-exports it so the documented script path keeps
working from a checkout, while installed environments (pip, uvx) load schemas and the
default catalog from package data.
"""
...
def _package_data(*parts: str) -> str:
    return resources.files("constitutional_cms").joinpath("data", *parts).read_text(encoding="utf-8")


def load_schema(schema_name: str) -> dict[str, Any]:
    """Load a bundled JSON Schema from package data."""
    return json.loads(_package_data("schemas", schema_name))


def load_default_catalog() -> dict[str, Any]:
    """Load the bundled CheckCatalogV1 shipped with this release."""
    return yaml.safe_load(_package_data("catalog", "check_catalog_v1.yaml"))
...
def main() -> int:
    ...
    parser.add_argument("--catalog", default=None, help="Catalog path (defaults to the bundled catalog)")
    ...
    try:
        catalog = load_data(Path(args.catalog)) if args.catalog else load_default_catalog()
        receipt = evaluate(catalog, load_data(Path(args.evidence)), args.as_of)
